...
type ProfileTab = "videos" | "photos" | "saved" | "liked" | "duets" | "drafts";
...
export default function ProfilePage({
  ...
  const [activityStatus, setActivityStatus] = useState<string>("Active now");
  ...
  useEffect(() => {
    ...
    backend
      .getActivityStatus(myPrincipal)
      .then((status) => {
        if (status) setActivityStatus(String(status));
      })
      .catch(() => {});
    ...
    Promise.all([
      ...
      backend.getCoverPhoto(myPrincipal).catch(() => null),
    ])
      .then(
        async ([
          profileOpt,
          ...
          // Load cover photo from backend
          if (
            coverPhotoKey &&
            typeof coverPhotoKey === "string" &&
            coverPhotoKey.startsWith("sha256:") &&
            thumbStorageClient
          ) {
            try {
              const url = await thumbStorageClient.getDirectURL(coverPhotoKey);
              setCoverUrl(url);
            } catch {}
          } else if (
            coverPhotoKey &&
            typeof coverPhotoKey === "string" &&
            coverPhotoKey.length > 0
          ) {
            setCoverUrl(coverPhotoKey);
          }
  ...
  // Load drafts
  useEffect(() => {
    if (activeTab !== "drafts" || draftsLoaded || !backend) return;
    backend
      .getReelDrafts()
      .then(async (rawDrafts) => {
        const resolved = await Promise.all(
          (
            ...
            let thumbUrl = d.thumbnailKey || "";
            if (thumbStorageClient && d.thumbnailKey?.startsWith("sha256:")) {
              try {
  ...
  }, [activeTab, draftsLoaded, backend, thumbStorageClient]);
  ...
  return (
    ...
    >
      ...
      {/* Activity Status — from backend */}
      <div className="px-4 pb-1">
        <span
          className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${activityStatus === "online" || activityStatus === "Active now" ? "text-green-400 bg-green-400/10" : "text-[#8B95A3] bg-[#1A1F26]"}`}
        >
          {activityStatus === "online" || activityStatus === "Active now"
            ? "🟢 Active now"
            : `🕐 ${activityStatus}`}
        </span>
      </div>
        ...
        <div className="flex px-2 min-w-max">
          {(
            [
              ...
              { id: "drafts", icon: <Clock size={15} />, label: "Drafts" },
            ] as const
          ).map(({ id, icon, label }) => (
            <button
              key={id}
      ...
      <div className="px-3 pt-3 pb-6">
        ...
        {activeTab === "drafts" &&
          (!draftsLoaded ? (
            <div
              className="flex justify-center py-16"
              data-ocid="profile.drafts.loading_state"
            >
              <div className="w-7 h-7 rounded-full border-2 border-[#22D3EE] border-t-transparent animate-spin" />
            </div>
          ) : drafts.length === 0 ? (
            <div
              className="flex flex-col items-center justify-center py-16 gap-3"
              data-ocid="profile.drafts.empty_state"
            >
              <Clock size={40} className="text-[#2A3038]" />
              <p className="text-[#8B95A3] text-sm text-center">
                No drafts yet. Save a reel as draft from the 3-dot menu.
              </p>
            </div>
          ) : (
            <div className="flex flex-row gap-3 overflow-x-auto pb-2 [&::-webkit-scrollbar]:hidden">
              {drafts.map((draft, i) => (
                <motion.div
                  ...
                  className="relative flex-shrink-0 rounded-xl overflow-hidden bg-[#1A1F26] cursor-pointer"
                  style={{ width: 140, aspectRatio: "9/16" }}
                  data-ocid={`profile.drafts.item.${i + 1}`}
                >
                  {draft.thumbUrl ? (
                    <img
                      src={draft.thumbUrl}
                    ...
                    <div className="absolute inset-0 bg-[#2A3038] flex items-center justify-center">
                      <Clock size={24} className="text-[#8B95A3]" />
                    </div>
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
                  <div className="absolute top-1.5 left-1.5 bg-[#F59E0B]/90 rounded-full px-1.5 py-0.5 flex items-center gap-0.5">
                    <Clock size={9} className="text-black" />
                  ...
                  <button
                    ...
                    onClick={async (e) => {
                      ...
                      if (!backend) return;
                      try {
                        await backend.deleteReelDraft(draft.id);
                        setDrafts((prev) =>
                    ...
                    data-ocid="profile.draft.delete.button"
                  >
                    <Trash2 size={10} className="text-[#FF3B5C]" />
                  </button>
                </motion.div>
              ))}
            </div>
