...
export default function ExplorePage({
  ...
  // Load trending hashtags from backend
  useEffect(() => {
    if (!backend) return;
    backend
      .getTrendingHashtags()
      .then((tags) =>
        setTrendingHashtags(
          (tags as Array<{ tag: string; count: bigint }>).slice(0, 10),
        ),
      )
      .catch(() => {});
  }, [backend]);

  // Load top creators for leaderboard using getLeaderboard()
  // biome-ignore lint/correctness/useExhaustiveDependencies: storage clients stable
  useEffect(() => {
    if (!backend) return;
    const loadLeaderboard = async () => {
      try {
        const leaderboard = await backend.getLeaderboard().catch(() => []);
        const resolved = await Promise.allSettled(
          (leaderboard as any[]).slice(0, 10).map(async (entry) => {
            const pStr =
            ...
            try {
              const { Principal } = await import("@icp-sdk/core/principal");
              const opt = await backend.getProfile(Principal.fromText(pStr));
              if (opt.__kind__ === "Some" && opt.value.avatarKey) {
            ...
            return {
              ...
              avatarUrl,
              followerCount: entry.followerCount ?? 0n,
              totalViews: entry.totalViews ?? 0n,
            };
          }),
        );
        const data = resolved
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#0F1216]">
      ...
      <div className="sticky top-0 bg-[#0F1216]/95 backdrop-blur px-4 pt-4 pb-0 z-10">
                  ...
                  <button
                    ...
                    onMouseDown={() => {
                      if (result.type === "hashtag") {
                        ...
                        onViewProfile(result.principal);
                      } else {
        ...
        {/* Trending hashtag pills row — real data from backend */}
        {!query.trim() && (
        ...
        <div className="px-4 mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {(trendingHashtags.length > 0
              ...
              : [
                  { tag: "fyp", count: 842n },
                  { tag: "viral", count: 630n },
                  { tag: "dance", count: 520n },
                  { tag: "comedy", count: 410n },
                  { tag: "trending2026", count: 380n },
                  { tag: "vibeflow", count: 290n },
                  { tag: "music", count: 254n },
                  { tag: "sports", count: 190n },
                  { tag: "gaming", count: 175n },
                  { tag: "art", count: 142n },
                ]
            ).map((item) => {
              ...
              return (
                <button
                  key={item.tag}
                  type="button"
                  onClick={() => onViewHashtag?.(item.tag)}
