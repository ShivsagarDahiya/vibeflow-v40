...
actor {
  ...
  // Expanded UserSettings with 18 fields
  public type UserSettings = {
    isPrivate : Bool;
    notificationsEnabled : Bool;
    theme : Text;               // dark | light | auto
    language : Text;            // en | hi | es | fr | de | ja
    contentFilter : Text;       // all | moderate | strict
    ageRestriction : Bool;
    blockedHashtags : Text;     // comma-separated
    blockedUsers : Text;        // comma-separated principals
    twoFactorEnabled : Bool;
    emailNotifications : Bool;
    pushNotifications : Bool;
    autoPlayVideos : Bool;
    videoQuality : Text;        // HD | SD | Auto
    activityStatusVisible : Bool;
    readReceiptsEnabled : Bool;
    muteNotificationSound : Bool;
    dmPrivacy : Text;           // everyone | matches | none
  ...
  // New stable state
  stable var _reelDrafts : [(Text, ReelDraft)] = [];
  stable var _messageReactions : [(Text, MessageReaction)] = [];
  stable var _pinnedMessages : [(Text, PinnedMessage)] = [];
  stable var _activityStatuses : [(Text, ActivityStatus)] = [];
  ...
  // New counters
  stable var _reelDraftCounter : Nat = 0;
  stable var _messageReactionCounter : Nat = 0;
  ...
  // New runtime maps
  var reelDrafts : Map.Map<Text, ReelDraft> = Map.fromArray(_reelDrafts);
  var messageReactions : Map.Map<Text, MessageReaction> = Map.fromArray(_messageReactions);
  var pinnedMessages : Map.Map<Text, PinnedMessage> = Map.fromArray(_pinnedMessages);
  var activityStatuses : Map.Map<Text, ActivityStatus> = Map.fromArray(_activityStatuses);
  ...
  system func preupgrade() {
    ...
    _reelDrafts := reelDrafts.toArray();
    _messageReactions := messageReactions.toArray();
    _pinnedMessages := pinnedMessages.toArray();
    _activityStatuses := activityStatuses.toArray();
  };
  ...
  func nextReelDraftId() : Text { _reelDraftCounter += 1; "rd" # _reelDraftCounter.toText() };
  func nextMessageReactionId() : Text { _messageReactionCounter += 1; "mr" # _messageReactionCounter.toText() };
  ...
  // Expanded updateUserSettings — all 18 fields as optional (uses ?T with fallback to current/default)
  ...
  public shared ({ caller }) func storeCallSignal(callee : Principal, signalType : Text, payload : Text, callType : Text) : async Text {
    // Clean up old signals (> 2 minutes old)
  ...
  // ===== LEADERBOARD =====
  public query func getLeaderboard() : async [{ principal : Principal; username : Text; videoCount : Nat; followerCount : Nat; totalViews : Nat }] {
    var stats : [{ principal : Principal; username : Text; videoCount : Nat; followerCount : Nat; totalViews : Nat }] = [];
    for ((_, u) in users.entries()) {
      var videoCount = 0;
      var totalViews = 0;
      var followerCount = 0;
      for ((_, v) in videos.entries()) {
        if (Principal.equal(v.creator, u.principal)) {
          videoCount += 1;
          totalViews += v.views;
        };
      };
      for ((_, list) in following.entries()) {
        if (hasPrincipal(list, u.principal)) { followerCount += 1 };
      };
      stats := stats.concat([{ principal = u.principal; username = u.username; videoCount; followerCount; totalViews }]);
    };
    // Sort by totalViews descending, take top 10
    let sorted = stats.sort(func(a, b) = Nat.compare(b.totalViews, a.totalViews));
    let size = sorted.size();
    sorted.sliceToArray(0, Nat.min(10, size));
  };

  // ===== ACTIVITY STATUS =====
  public shared ({ caller }) func updateActivityStatus(status : Text) : async () {
    activityStatuses.add(pkey(caller), { status; lastSeen = Time.now() });
  };

  public query func getActivityStatus(user : Principal) : async Text {
    switch (activityStatuses.get(pkey(user))) {
      case null "offline";
      case (?s) {
        let now = Time.now();
        let diff = now - s.lastSeen;
        // Within 30 seconds (30_000_000_000 ns) = online
        // Within 5 minutes (300_000_000_000 ns) = away
        if (diff < 30_000_000_000) { "online" }
        else if (diff < 300_000_000_000) { "away" }
        else { "offline" };
      };
    };
  };
  ...
  // ===== REEL DRAFTS =====
  public shared ({ caller }) func saveReelDraft(videoKey : Text, thumbnailKey : Text, title : Text, description : Text, hashtags : Text) : async Text {
    let id = nextReelDraftId();
    reelDrafts.add(id, { id; creator = caller; videoKey; thumbnailKey; title; description; hashtags; createdAt = Time.now() });
    id;
  };

  public shared query ({ caller }) func getReelDrafts() : async [{ id : Text; videoKey : Text; thumbnailKey : Text; title : Text; description : Text; hashtags : Text; createdAt : Int }] {
    var arr : [{ id : Text; videoKey : Text; thumbnailKey : Text; title : Text; description : Text; hashtags : Text; createdAt : Int }] = [];
    for ((_, d) in reelDrafts.entries()) {
      if (Principal.equal(d.creator, caller)) {
        arr := arr.concat([{ id = d.id; videoKey = d.videoKey; thumbnailKey = d.thumbnailKey; title = d.title; description = d.description; hashtags = d.hashtags; createdAt = d.createdAt }]);
      };
    };
    arr.sort(func(a, b) = Int.compare(b.createdAt, a.createdAt));
  };

  public shared ({ caller }) func deleteReelDraft(draftId : Text) : async Bool {
    switch (reelDrafts.get(draftId)) {
      case null false;
      case (?d) {
        if (not Principal.equal(d.creator, caller)) return false;
        reelDrafts.remove(draftId);
        true;
      };
    };
  };
  ...
  // ===== MESSAGE REACTIONS =====
  public shared ({ caller }) func addMessageReaction(messageId : Text, emoji : Text) : async () {
    // Remove existing reaction from same user on same message
    for ((rid, r) in messageReactions.entries()) {
      if (r.messageId == messageId and Principal.equal(r.sender, caller)) {
        messageReactions.remove(rid);
      };
    };
    let id = nextMessageReactionId();
    messageReactions.add(id, { id; messageId; sender = caller; emoji; createdAt = Time.now() });
  };

  public query func getMessageReactions(messageId : Text) : async [{ sender : Principal; emoji : Text }] {
    var arr : [{ sender : Principal; emoji : Text }] = [];
    for ((_, r) in messageReactions.entries()) {
      if (r.messageId == messageId) {
        arr := arr.concat([{ sender = r.sender; emoji = r.emoji }]);
      };
    };
    arr;
  };
  ...
  // ===== PINNED MESSAGES =====
  public shared ({ caller }) func pinMessage(messageId : Text) : async () {
    switch (messages.get(messageId)) {
      case null {};
      case (?msg) {
        // Only participants can pin
        let convId = msg.conversationId;
        let key = convId # "_pin_" # messageId;
        pinnedMessages.add(key, { messageId; conversationId = convId; pinnedAt = Time.now() });
      };
    };
  };

  public shared query ({ caller }) func getPinnedMessages(otherPrincipal : Principal) : async [Message] {
    let convId = conversationId(caller, otherPrincipal);
    var arr : [Message] = [];
    for ((_, pm) in pinnedMessages.entries()) {
      if (pm.conversationId == convId) {
        switch (messages.get(pm.messageId)) {
          case (?msg) { arr := arr.concat([msg]) };
          case null {};
        };
      };
    };
  ...
  // ===== REEL ANALYTICS =====
  public shared query ({ caller }) func getReelAnalytics(videoId : Text) : async { plays : Nat; likes : Nat; comments : Nat; shares : Nat; saves : Nat } {
    switch (videos.get(videoId)) {
      case null { { plays = 0; likes = 0; comments = 0; shares = 0; saves = 0 } };
      case (?v) {
        // Only return analytics; shares not yet tracked separately so return 0
        let likeCount = likes.get(videoId).get([]).size();
        var commentCount = 0;
        for ((_, c) in comments.entries()) { if (c.videoId == videoId) { commentCount += 1 } };
        var saveCount = 0;
        for ((_, savedList) in savedVideos.entries()) { if (hasText(savedList, videoId)) { saveCount += 1 } };
        { plays = v.views; likes = likeCount; comments = commentCount; shares = 0; saves = saveCount };
      };
    };
  };

  // ===== TRENDING HASHTAGS =====
  public query func getTrendingHashtags() : async [{ tag : Text; count : Nat }] {
    var tagCounts : Map.Map<Text, Nat> = Map.empty();
    for ((_, v) in videos.entries()) {
      for (tag in v.hashtags.values()) {
        let normalized = tag.toLower();
        let current = tagCounts.get(normalized).get(0);
        tagCounts.add(normalized, current + 1);
      };
    };
    var arr : [{ tag : Text; count : Nat }] = [];
    for ((tag, count) in tagCounts.entries()) {
      arr := arr.concat([{ tag; count }]);
    };
    let sorted = arr.sort(func(a, b) = Nat.compare(b.count, a.count));
    let size = sorted.size();
    sorted.sliceToArray(0, Nat.min(20, size));
  };
};
